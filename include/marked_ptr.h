...
public:
    ...
    element_type *raw() const { return reinterpret_cast<element_type *>(ptr_); }

    element_type *get() const { return reinterpret_cast<element_type *>(ptr_ & ~(1)); }
    ...
    friend bool operator==(const marked_ptr &left, const marked_ptr &right) { return left.ptr_ == right.ptr_; }

    friend bool operator!=(const marked_ptr &left, const marked_ptr &right) { return !(left == right); }

    friend bool operator<(const marked_ptr &left, const marked_ptr &right) { return left.ptr_ < right.ptr_; }

    friend bool operator>(const marked_ptr &left, const marked_ptr &right) { return right < left; }

    friend bool operator<=(const marked_ptr &left, const marked_ptr &right) { return !(right < left); }

    friend bool operator>=(const marked_ptr &left, const marked_ptr &right) { return !(left < right); }

    static marked_ptr pointer_to(element_type &value) { return marked_ptr(&value); }
