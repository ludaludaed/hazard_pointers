...
public:
    explicit IntrusiveHashtable(const bucket_traits &buckets = {}, const hasher &hash = {}, const key_equal &equal = {},
        ...
        : data_(NilNodeHolder{}, value_traits, buckets, KeyOfValue{}, hash, equal, SizeTraits{}) {
        Construct();
    }
...
private:
    ...
    inline node_ptr GetNilPtr() noexcept {
        return std::pointer_traits<node_ptr>::pointer_to(lu::get<NilNodeHolder>(data_).nil_node);
    }

    inline const_node_ptr GetNilPtr() const noexcept {
        return std::pointer_traits<const_node_ptr>::pointer_to(lu::get<NilNodeHolder>(data_).nil_node);
    }
...
public:
    ...
    void rehash(const bucket_traits &new_bucket_traits) {
        ...
        Algo::init(GetNilPtr());
        lu::get<SizeTraits>(data_).set_size(0);
...
private:
    lu::compressed_tuple<NilNodeHolder, ValueTraits, BucketTraits, KeyOfValue, KeyHash, KeyEqual, SizeTraits> data_{};
};
